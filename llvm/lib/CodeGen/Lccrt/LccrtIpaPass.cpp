...
template<typename T>
void
FunctionIPAResults::collectAliasInfo(T &aaResult)
{
    if (aliases.empty()) {
    ...
    for (size_t i = 0; i < values.size(); ++i) {
        for (size_t j = 0; j < values.size(); ++j) {
            if (aliases[i][j] == AliasResult::MayAlias) {
                aliases[i][j] = aaResult.alias(values[i], values[j]);
            }
        }
    }
}
...
INITIALIZE_PASS_BEGIN(LccrtIpaPass, "lccrt-ipa",
                      "Collect IPA Results For LCC", false, true)
...
LccrtIpaPass::runOnModule(Module &M)
{
    ...
    for (auto &i : ipaResults) {
        ...
        if (tbaa) {
            ...
            i.second.collectAliasInfo(tbaaResults);
        } else {
        ...
        i.second.collectAliasInfo(aaResults);
    }
#endif
