...
LccrtFunctionEmitter::makeFshl( User &O, lccrt_var_ptr res, lccrt_oi_ptr i)
{
    ...
    if ( isa<ScalableVectorType>( T1) ) {
        ...
        if ( isa<IntegerType>( TR) && is_pow2( bitsize) && (4 <= bitsize) && (bitsize <= 128) ) {
            char sf[256];
...
LccrtFunctionEmitter::makeIntAbs( User &O, lccrt_var_ptr res, lccrt_oi_ptr i)
{
    ...
    if ( isa<ScalableVectorType>( T1) ) {
        ...
        lccrt_var_ptr v1 = lccrt_var_new_local( f, lccrt_var_get_type( a1), 0);
        lccrt_var_ptr v2 = le.makeVarConstHex( lccrt_var_get_type( a1), 0);
        ...
        lccrt_oper_new_select( f, v0, a1, v1, res, i);
    } else {
        errorDump( &O);
    }
