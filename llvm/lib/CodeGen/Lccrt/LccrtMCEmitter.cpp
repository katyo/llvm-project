...
void
LccrtMCMachObjectTargetWriter::recordRelocation( MachObjectWriter *Writer, MCAssembler &Asm,
...
LccrtMCAsmBackend::createObjectWriter( raw_pwrite_stream &OS) const
{
    ...
    std::unique_ptr<MCObjectWriter> r = std::make_unique<ELFObjectWriter>(
        cast<MCELFObjectTargetWriter>(std::move(TW)), OS, true);
...
bool
LccrtMCAsmBackend::fixupNeedsRelaxation( const MCFixup &Fixup, uint64_t Value) const
{
